...
- [面板窗口化设计计划](windowing-plan.md) -- **可直接照着实现的完整方案**:把现在
  三个贴边固定面板(源码/参数/对象)改成**桌面 + 浮动窗口 + Dock**,并把右栏那两个
  标签页("参数 / 视图"与"过程")拆成各自独立的窗口(共四个).已拍板:①形态为
  ...
  四个窗口的数值(锚点式,含 1280×800 / 1920×1080 两组已核算的期望值),
  `WindowGeometry` / `WindowResize` / `WindowFrame` / `WindowManager` 的签名与
  算法,夹取/吸附/最大化公式,状态机与两个唯一写入点,`DslApp` 的逐条接线.
  **§5** 是文件清单(新增/修改/删除),DOM 契约与 CSS 规则归属;**§5.6 动编辑器
  前必读**(源码高亮层:文字透明,着色全靠背后一层,错一像素就表现为"编辑不了",
  列全五条对齐轴与不许碰的东西);**§8.1** 说明哪些能靠单测,哪些只能真机
  (DOM 桩不解析样式表,不做布局).
  **§11 是动手前必看的阻碍与订正**:七条硬性阻碍(页容器残留 `hidden` 导致窗口
  空白,示例浮层的 overflow 被窗口裁掉,Dock 通栏挡住底边手柄,诊断区高度基准,
  `.window-body` 的 flex 约束,`#right-panel` 空壳,三个派生变量)+ 七条方案自身
  写错并已订正的地方(E1 宿主查询,E3/E4 默认几何不该走 CSS,E5 浮层高度)+
  一份"查过但不是阻碍"的记录.
