...
- [曲面非有限(NaN)三角形的收口与已知边界](surface-nan-audit.md) -- 维护者审计
  ...
  **有意保留**的边界:L1 定义域边界/极点附近一条一个网格步宽的缺失带(判据只能
  ...
  通路-回归用例对照表.
- **渲染精度,双目标架构与桌面端(Linux)分离方案**(原文
  `docs/render-ui-migration-plan.md` 已按"过期文档"删除,见提交 `2db7b67`;
  以下为摘要留档,原文可从 `git show 2db7b67^:docs/render-ui-migration-plan.md`
  取回) --
...
- [面板窗口化设计计划](windowing-plan.md) -- **已落地(阶段 0–3)的实现方案**:
  ...
  **桌面 + 浮动窗口 + 顶部任务栏**,并把右栏那两页
  与"参数区/视图区"这两层共用都拆开,变成六个独立窗口:
  `source code` / `视图` / `参数` / `过程` / `实体对象` / `求值对象`(落地时先
  做成"对象"一窗,后来按提交 `9333355` 拆成实体/求值两个).已拍板:①形态为
  ...
  六个窗口的数值(锚点式,含 1280×800 / 1920×1080 两组已核算的期望值;
  `view` / `process` / 两个对象窗口的下沿共用底边线 `dH-edgeGap`,`dockReserve`
  改成从**顶部**让位,其余窗口的底边
  必须在它之上--订正见 E21),
...
- [UI 库分离设计计划](ui-library-extraction-plan.md) -- 规划文档:回答"把界面抽成
  ...
  §6 抽库后的目录树(库六个子包 + 应用侧 `ui/` `adapters/`;该节原文按当时的
  名字写作 `views/`,补记七已改回 `ui/`),§7 先 npm
  ...
  **附录 C** 记搬库时最容易丢的六件事(C1 `applyUiConfig()` 必须先于构造,否则
  ...
  状态:**已落地:P0–P4 完成,库已独立成 repo 并发布到 npm**;两条决策都已拍板,**本文已无待定项**:
...
- **三期完整规划与设置**(原文 `docs/plan3.md` 已按"过期文档"删除,见提交
  `6ad16b0`;微分方程三期已落地,总口径见[不定积分与微分方程:设计计划]
  (calculus-suite-plan.md),用户文档见[微分方程使用指南](ode-guide.md);原文可从
  `git show 6ad16b0^:docs/plan3.md` 取回) -- 执行清单:微分方程(DSL 语法,
  导数记号归一,八类方程的判定与步法,解族/斜率场可视化,两层编译,IR 与展示,
  示例清单),四期收尾增强,测试计划,验收口径,以及三个待拍板的设计点
  (P1 解族怎么画 / P2 积分常数怎么处理 / P3 隐式解怎么办)的备选方案与推荐.
