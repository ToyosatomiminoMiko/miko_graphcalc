...
状态:**已落地(P0–P4 完成,库已独立成 repo 并发布到 npm;见下面的"落地进展"
与补记一–七)**.正文里"规划稿"口吻的耦合点清单仍标着文件与数目,是写作当时
(核对时间:2026-09-22 的工作区,`src/ui` 共 14,589 行)的快照,可以逐条对着
当时的工作区核对;**附录 A 给出复现这些数字的全部命令**,数字若有出入以重跑
结果为准.
...
>
> **补记七(目录名回归 `src/ui`,2026-09):** P2 把应用视图从 `src/ui/` 改名到
> `src/views/`,但 `views` 与本仓库的 three.js 侧(`render/` + `#viewport`)
> 容易混:**`src/ui/view/` 指的是"视图窗口"(相机/坐标轴/网格/曲面的控件),
> 不是 3D 视口**,而 `view` 这个词在别处又指视口.现在改回 `src/ui/`:应用侧
> 界面只保留"声明(有什么节点)+ 行为(控制器)",样式按下面的分层归库.
> 补记里的历史路径按当时状态保留,不在正文里回改.
