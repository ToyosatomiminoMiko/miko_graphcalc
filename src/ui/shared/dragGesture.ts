 ...
 * - `panels/PanelController`:左右面板宽度,底部面板高度(通过
 *   `[data-resize-panel]` 上的三根分隔条);
 * - `panels/RightSplitController`:右面板内部"参数区 / 视图区"的高度比例.
 *
 * 为什么必须共用:两处原先各写一遍"起手 -> 挂 move/up -> 收尾复位
 * `document.body.style.cursor`",而两份的不一致处就是理解偏差的藏身处:
 * 一处的收尾监听绑在分隔条自己身上(靠 `setPointerCapture` 兜住),另一处绑在
 * window 上再手动摘;一处把光标写死成 `'ns-resize'`,另一处读分隔条自己的
 * 计算样式.收成一份之后,光标只有一个来源(CSS),收尾只有一条路径(signal).
 *
 * 两个刻意的设计点:
...
export interface DragGestureHandlers {
     ...
     * `is-dragging`.消费者用它表达"当前这个分隔条不可拖"(如面板已折叠),
     * 而不是在 `onDelta` 里反复判断.
     */
    canStart?(): boolean;
 ...
 * @param handlers      回调
 * @param onCursorReset 收尾后恢复光标的方式;默认清空 `document.body.style.cursor`
 */
export function bindDragGesture(
    ...
    const onPointerMove = (event: PointerEvent): void => {
        ...
        lastX = event.clientX;
        lastY = event.clientY;
        handlers.onDelta(deltaX, deltaY);
    };
    ...
    handle.addEventListener('pointerdown', (event: PointerEvent) => {
        if (active) return;
        if (handlers.canStart?.() === false) return;
