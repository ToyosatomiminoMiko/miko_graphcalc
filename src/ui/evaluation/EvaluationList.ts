/**
 * 求值列表(footer 右栏):分析/积分/求交三个子列表的集合.
 *
 * 这里只做"右栏"这一层的事:
 * 1. 三个 kind 各挂一个 {@link EvaluationSection}(DOM 行缓存/顺序/展开态/
 *    数值缓存/异步回填的统一实现);
 * 2. 把场景里的三类任务分别渲染进各自的容器,显隐回调接到对应的切换入口;
 * 3. 对外转发渲染层的异步结果回调(积分/求交;分析是编译期同步值,没有入口).
 *
...
/** 右栏三个子列表容器;用具名对象而不是三个同类型的 HTMLElement 位置参数. */
