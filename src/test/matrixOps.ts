 ...
 *
 * 单独放在 `src/test/` 而不是让每个测试各自 `import` 生产模块:一是
 * `render/**` 不必为了测试反向依赖 `compiler/**`(生产里渲染层只通过注入的
 * `MatrixOps` 接口拿后端),二是"测试注入生产后端"这条约定只有一处声明.
 *
 * `test/setupWasm.ts` 已在启动时同步初始化 WASM,所以模块级构造即可直接调用.
 */
